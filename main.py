...
async def create_task(task: task_schema, db: Session = Depends(get_database_session)):
   new_task = Task(
...
async def get_task(id:int,db: Session = Depends(get_database_session)):
   task = db.query(Task).get(id)
   return task

@app.patch("/task/{id}", response_model = task_schema, status_code=200)
async def update_task(id:int, task:task_schema, db: Session = Depends(get_database_session)):
...
async def delete_task(id:int, db: Session = Depends(get_database_session)):
    db_task = db.query(Task).get(id)
